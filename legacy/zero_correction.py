...
parser.add_argument(
    "trigger_index",
    type=int,
    help="Index of the trigger detector. We will only choose the events where this detector is triggered.",
)
parser.add_argument(
    "--pos_matrix",
    type=str,
...
if trigger_index < 0 or trigger_index >= n_det:
    raise ValueError(
        f"trigger_index {trigger_index} out of range. Valid range: 0-{n_det - 1}"
    )
